...
{
    return db()->query('SELECT id, username, full_name, email, role, is_active, last_login_at, created_at FROM users ORDER BY role DESC, full_name ASC')->fetchAll();
}

function get_user(int $id): ?array
{
    $stmt = db()->prepare('SELECT id, username, full_name, email, role, is_active, last_login_at, created_at FROM users WHERE id = ?');
    $stmt->execute([$id]);
    $row = $stmt->fetch();
    return $row ?: null;
}
...
function handle_image_upload(array $file, string $subdir): ?string
{
    ...
    $uploadDir = public_path('produtos/' . $folderName);
    ...
    return 'produtos/' . $folderName . '/' . $filename;
}
